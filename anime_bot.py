...
PAYMENT_CARD_OWNER = "F. F. Familiya"
VIP_PLANS = [
    ("7 kunlik VIP", "10 000 so'm"),
    ("30 kunlik VIP", "30 000 so'm"),
    ("Cheksiz VIP", "80 000 so'm"),
]
...
async def cmd_admin(message: Message):
    ...
    await message.answer(
        ...
        "/givevip USER_ID KUNLAR — VIP berish\n"
...
async def cmd_addep_help(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        return
    await message.answer(
        "❗️ Qism qo'shish uchun: videoni to'g'ridan-to'g'ri botga yuboring, "
        "video ostiga (caption) yozing:\n<code>/addep KOD FASL QISM</code>\n"
        "Masalan: <code>/addep 25 1 1</code>",
        parse_mode="HTML",
    )
...
async def cmd_givevip(message: Message, command: CommandObject):
    if message.from_user.id not in ADMIN_IDS:
        return
    if not command.args:
        await message.answer("❗️ Foydalanish: /givevip USER_ID KUNLAR_SONI (yoki 'forever')")
        return
    try:
        ...
        await message.answer("❗️ Foydalanish: /givevip USER_ID KUNLAR_SONI")
        return

    ensure_user(target_id, "")
    until = "forever" if duration == "forever" else (datetime.now() + timedelta(days=int(duration))).isoformat()
